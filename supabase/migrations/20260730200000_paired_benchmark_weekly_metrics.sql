-- Paired benchmark + weekly metric snapshots.
--
-- Makes the landing-page benchmark claims derivable from telemetry instead of
-- hardcoded:
--   1. adversarial_runs learns which ARM a run measured (certified | baseline
--      | frozen_v1), so the "ungraded vs SAK" comparison and the frozen-v1
--      drift control live in the same audited table as every other run.
--   2. get_attack_class_benchmark() aggregates recent runs into the four
--      headline attack classes with Wilson 95% lower bounds — the number the
--      site should publish is wilson_lb, never the raw average.
--   3. package_weekly_metrics accumulates the per-week series behind the
--      "drift vs climb" 12-week chart. Populated by a weekly pg_cron job.

-- 1. Arm column on adversarial_runs -----------------------------------------
...
-- 2. Attack-class benchmark over recent runs --------------------------------
-- Explodes by_category jsonb into the four headline classes and returns, per
-- (class, arm): totals, raw rate and the Wilson 95% lower bound. SECURITY
-- INVOKER + read-only: it only aggregates what RLS already exposes.
...
-- 3. Weekly metric snapshots -------------------------------------------------
-- One row per (package, week, track). track='current' follows the live
-- version; track='frozen_v1' is the drift control: the same suite re-run
-- weekly against the package's first published version, WITHOUT SkillForge
-- patches. current climbing while frozen_v1 sinks IS the measured value of
-- the evolution loop — the 12-week chart, from real data.
...
declare
  affected int := 0;
  wk date := date_trunc('week', now() - interval '7 days')::date; -- last full week
begin
  ...
  -- frozen_v1 rows come from adversarial runs tagged subject_arm='frozen_v1'
  -- (posted by the weekly re-run of each package's first published version).
  insert into public.package_weekly_metrics
...
-- Every Monday 02:10 UTC, snapshot the week that just closed.
...
-- 4. Findings MTTR ------------------------------------------------------------
-- "Mean time to patch" must be measured, not asserted: hours from a finding
-- going public to the fixed version's signed release.

create or replace view public.findings_mttr as
select
