CREATE TABLE IF NOT EXISTS public.crm_lifecycle_state (
  user_id UUID PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
  stage TEXT NOT NULL DEFAULT 'new',
  first_seen TIMESTAMPTZ NOT NULL DEFAULT now(),
  last_active_at TIMESTAMPTZ,
  last_email_at TIMESTAMPTZ,
  emails_sent_7d INTEGER NOT NULL DEFAULT 0,
  crm_unsubscribed BOOLEAN NOT NULL DEFAULT false,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

GRANT SELECT ON public.crm_lifecycle_state TO authenticated;
GRANT ALL ON public.crm_lifecycle_state TO service_role;
ALTER TABLE public.crm_lifecycle_state ENABLE ROW LEVEL SECURITY;

CREATE POLICY "crm_state_select_own" ON public.crm_lifecycle_state
  FOR SELECT TO authenticated
  USING (auth.uid() = user_id OR public.has_role(auth.uid(), 'admin'));

CREATE TRIGGER crm_lifecycle_state_touch
  BEFORE UPDATE ON public.crm_lifecycle_state
...
CREATE TABLE IF NOT EXISTS public.crm_message_log (
  ...
  roi_snapshot JSONB NOT NULL DEFAULT '{}'::jsonb,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

GRANT SELECT ON public.crm_message_log TO authenticated;
GRANT ALL ON public.crm_message_log TO service_role;
ALTER TABLE public.crm_message_log ENABLE ROW LEVEL SECURITY;

CREATE POLICY "crm_msg_select_own" ON public.crm_message_log
  FOR SELECT TO authenticated
  USING (auth.uid() = user_id OR public.has_role(auth.uid(), 'admin'));

CREATE INDEX IF NOT EXISTS crm_message_log_user_idx ON public.crm_message_log (user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS crm_message_log_template_idx ON public.crm_message_log (template, created_at DESC);
