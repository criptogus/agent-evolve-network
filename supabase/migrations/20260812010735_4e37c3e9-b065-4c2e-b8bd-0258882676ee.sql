-- Add an optional _user_id filter to crm_customers so per-user surfaces
-- (e.g. the home "Your value so far" card) fetch exactly one row instead of
-- scanning the whole customer list. Also lets a user read their own row.
DROP FUNCTION IF EXISTS public.crm_customers(INTEGER, INTEGER);

CREATE FUNCTION public.crm_customers(_limit INTEGER DEFAULT 500, _offset INTEGER DEFAULT 0, _user_id UUID DEFAULT NULL)
RETURNS TABLE (
  user_id UUID,
  email TEXT,
  display_name TEXT,
...
AS $$
  ...
  WHERE (auth.uid() IS NULL OR public.has_role(auth.uid(), 'admin') OR u.id = auth.uid())
    AND (_user_id IS NULL OR u.id = _user_id)
  ORDER BY u.created_at DESC
  LIMIT _limit OFFSET _offset
$$;

REVOKE ALL ON FUNCTION public.crm_customers(INTEGER, INTEGER, UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.crm_customers(INTEGER, INTEGER, UUID) TO authenticated, service_role;